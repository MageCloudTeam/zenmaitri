...
(function(){
  ...
  // theme.handleResize = function () {
  //   window.on('resize', theme.utils.debounce(250, function () {
  //     location.reload();
  //   }));
  // }
  ...
  theme.Drawers = (function() {
    ...
    Drawers.prototype = Object.assign({}, Drawers.prototype, {
      ...
      open: function(evt, returnFocusEl) {
        ...
        var promote = document.querySelector('.src-assets-styles-style__modalActivatorContent--23xZ_');
        if (promote) {
          if (promote.closest('span')) {
            promote.closest('span').style.display = 'none';
          } else {
      ...
      close: function(evt) {
        ...
        if (promote) {
          if (promote.closest('span')) {
            promote.closest('span').style.display = 'block';
          } else {
    ...
    QtySelector.prototype = Object.assign({}, QtySelector.prototype, {
      init: function() {
        ...
        this.plus.addEventListener('click', function() {
          this.quantity = this._getQty();
          this._change(this.quantity + 1);
        }.bind(this));
        ...
        this.input.addEventListener('change', function(evt) {
          this._change(this._getQty(this.input.value));
        }.bind(this));
      },

      _getQty: function(qty = this.quantity) {
      ...
      _change: function(qty) {
        ...
        // this.input.value = qty; - update all instead
        var self = this;
        this.wrapper.querySelectorAll(selectors.input)
            .forEach(function (el) {
              el.value = self.quantity;
            })
  ...
  theme.initQuickShop = function() {
    var ids = [];
    var products = document.querySelectorAll('.grid-product:not(.quick-view-initialised)');
    ...
    products.forEach(product => {
      var className = 'quick-view-initialised-' + product.handle;
  ...
  theme.buildProductGridItem = function(items, gridWidth, rowOf, imageSize) {
    var output = '';
    var allLabels = theme.settings.productLabels.map(l => l.trim());

    items.forEach(product => {
      var image = theme.buildProductImage(product, imageSize);
      var comparePrice = `
        <span class="visually-hidden">${theme.strings.regularPrice}</span>
        <span class="grid-product__price--original">${theme.Currency.formatMoney(product.compare_at_price)}</span>
        <span class="visually-hidden">${theme.strings.salePrice}</span>
      `;
      var tags = product.tags.map(function (tag) {
        var matchingLabel = allLabels.find(l => l.indexOf(tag) > -1);
        if (matchingLabel || tag.indexOf('_label_') > -1) {
          var color = matchingLabel.split(':')[1];
          tag = tag.split(':')[0];
          return `<div 
            class="grid-product__tag grid-product__tag--custom"
            ${color ? `style="background-color: ${color}"`: ''}
          >${tag.replace('_label_', '')}</div>`
        } else {
          return '';
        }
      }).filter(t => t !== '').join('');
      ...
      var markup = `
        <div class="grid__item grid-product ${gridWidth} aos-animate" data-aos="row-of-${rowOf}" data-product-handle="${product.handle}" data-product-id="${product.id}">
          <div class="grid-product__content">
            ${tags.length ? `<div class="grid-product__tags-container">${tags}</div>` : ''}
            <a href="${product.url}" class="grid-product__link">
              ...
              <div class="grid-product__meta">
                ...
                <div class="grid-product__price">
                    ${product.compare_at_price > product.price ? comparePrice : ''}
                    ${product.price_varies ? 
                        theme.strings.priceFrom
                            .replace('{{ price }}', theme.Currency.formatMoney(product.price_min)) 
                        : theme.Currency.formatMoney(product.price)
                    }
                    ${theme.settings.quickView ? `<div class="product__actions">
                        <div class="quick-product__btn quick-product__btn--not-ready js-modal-open-quick-modal-${product.id} small--hide">
                          <span class="quick-product__label">${theme.strings.quickShopText}</span>
                        </div>
                        <span class="shopify-product-reviews-badge" data-id="${product.id}"></span>
                    </div>` : `<span class="shopify-product-reviews-badge" data-id="${product.id}"></span>`}
                </div>
              </div>
            </a>
          </div>
        ...
        if (config.combineTags) {
          if (parent.classList.contains(classes.activeTag)) {
            ...
            if (el.closest('li').classList.contains(classes.removeTagParent)) {
              ...
              newTagLink.classList.add('collection-filter__remove');
      ...
      openModalProduct: function() {
        ...
        if (!this.settings.modalInit) {
          ...
          fetch(url).then(function(response) {
            ...
            if (this.inModal) {
              const loader = this.container.querySelector('.form--loading');
              if (loader) loader.classList.remove('form--loading');
            }
